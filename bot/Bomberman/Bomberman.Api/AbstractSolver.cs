...
namespace Bomberman.Api
{
    public abstract class AbstractSolver
    {
        ...
        public void Play()
        {
            if (_useFile)
            {
                using (var log = new StreamReader(_filePath))
                {
                    var boardString = log.ReadToEnd();
                    var board = new Board(boardString, _useFile);
